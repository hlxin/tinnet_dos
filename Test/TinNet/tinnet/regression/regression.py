...
class Regression:
    ...
    def eval_train_model(self, **kwargs):
        ...
        for i, (input, target, batch_cif_ids) in enumerate(self.train_loader):
            ...
            if self.phys_model =='moment':
                output, parm, zeta, crys_fea = Tight_binding.moment(
                    self,
                    cnn_output,
                    cnn_output_crys,
                    **dict(**kwargs,
            ...
            np.savetxt('parm_train_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.txt', parm.detach().cpu().numpy())
            
            np.save('zeta_train_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.npy', zeta.detach().cpu().numpy())
            
            np.save('crys_fea_train_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.npy', crys_fea.detach().cpu().numpy())
            
            with open('train_results_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.csv', 'w') as f:
                writer = csv.writer(f)
                for cif_id, target, pred in zip(batch_cif_ids, target, output):
                    writer.writerow((cif_id, target[0].detach().cpu().numpy(), pred[0].detach().cpu().numpy()))
            tmp += 1
    ...
    def eval_val_model(self, **kwargs):
        ...
        for i, (input, target, batch_cif_ids) in enumerate(self.val_loader):
            ...
            np.savetxt('parm_validation_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.txt', parm.detach().cpu().numpy())
            
            np.save('zeta_validation_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.npy', zeta.detach().cpu().numpy())
            
            np.save('crys_fea_validation_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.npy', crys_fea.detach().cpu().numpy())
            
            with open('validation_results_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.csv', 'w') as f:
                writer = csv.writer(f)
                for cif_id, target, pred in zip(batch_cif_ids, target, output):
                    writer.writerow((cif_id, target[0].detach().cpu().numpy(), pred[0].detach().cpu().numpy()))
            tmp += 1
    ...
    def eval_test_model(self, **kwargs):
        ...
        for i, (input, target, batch_cif_ids) in enumerate(self.test_loader):
            ...
            np.savetxt('parm_test_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.txt', parm.detach().cpu().numpy())
            
            np.save('zeta_test_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.npy', zeta.detach().cpu().numpy())
            
            np.save('crys_fea_test_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.npy', crys_fea.detach().cpu().numpy())
            
            with open('test_results_idx_val_' + str(self.idx_validation) + '_idx_test_' + str(self.idx_test) + '_' + str(tmp) + '.csv', 'w') as f:
                writer = csv.writer(f)
                for cif_id, target, pred in zip(batch_cif_ids, target, output):
                    writer.writerow((cif_id, target[0].detach().cpu().numpy(), pred[0].detach().cpu().numpy()))
            tmp += 1
